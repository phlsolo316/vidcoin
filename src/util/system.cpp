...
const char * const VIDCOIN_CONF_FILENAME = "vidcoin.conf";
const char * const VIDCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/vidcoin/vidcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //vidcoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", VIDCOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\VIDCoin
    // macOS: ~/Library/Application Support/VIDCoin
    // Unix-like: ~/.vidcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VIDCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/VIDCoin";
#else
    // Unix-like
    return pathRet / ".vidcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", VIDCOIN_CONF_FILENAME);
