...
#include <qt/vidcoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a VIDCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new VIDCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new VIDCoinAddressCheckValidator(parent));
}

bool parseVIDCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no vidcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("vidcoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!VIDCoinUnits::parse(VIDCoinUnits::VID, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseVIDCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseVIDCoinURI(uriInstance, out);
}

QString formatVIDCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("vidcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(VIDCoinUnits::format(VIDCoinUnits::VID, info.amount, false, VIDCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openVIDCoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", VIDCOIN_CONF_FILENAME));
    ...
    /* Open vidcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "VIDCoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "VIDCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("VIDCoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for VIDCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "vidcoin.desktop";
    return GetAutostartDir() / strprintf("vidcoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a vidcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=VIDCoin\n";
        else
            optionFile << strprintf("Name=VIDCoin (%s)\n", chain);
