...
The VIDCoin Core daemon can be configured to act as a trusted "border
router", implementing the vidcoin wire protocol and relay, making
...
The ZeroMQ feature in VIDCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building vidcoind:
...
For instance:
    ...
    $ vidcoind -zmqpubhashtx=tcp://127.0.0.1:236357 \
               -zmqpubhashtx=tcp://192.168.1.2:236357 \
               -zmqpubrawtx=ipc:///tmp/vidcoind.tx.raw \
...
These options can also be provided in vidcoin.conf.
...
From the perspective of vidcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into vidcoind directly. Furthermore, no information is
...
using. VIDCoind appends an up-counting sequence number to each
